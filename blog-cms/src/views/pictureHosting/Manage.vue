	...
	export default {
		...
		methods: {
			...
			*/
			...
			imgUrl(file) {
				return this.isCDN ? `https://cdn.jsdelivr.net/gh/${this.userInfo.login}/${this.activeRepos}@master/${file.path}` : file.download_url
			},
			copy(type, file) {
				// type 1 cdn link  2 Markdown
				let imgUrl = `https://cdn.jsdelivr.net/gh/${this.userInfo.login}/${this.activeRepos}@master/${file.path}`
				let copyCont = imgUrl
				if (type == 2) {
